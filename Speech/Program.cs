...
/*
 * The Audio support in Microsoft.Extensions.AI is experimental
 * and therefore subject to breaking changes
 * 
...
internal class Program
{
    static async Task Main(string[] args)
    {
        ...
        var host = Host.CreateDefaultBuilder(args)
            ...
            {
                services.AddSingleton<ISpeechToTextClient>(sp =>
                     OpenAIClientExtensions.AsISpeechToTextClient(
...
public class ChatService : BackgroundService
{
        ...
        string? systemprompt)
    {
        ...
        SpeechToTextOptions speechToTextOptions = new()
        {
            // ...
        };
        ...
        do
        {
            ...
            byte[] blob = File.ReadAllBytes("audio.webm");  // captured audio file
            DataContent audioData = new(blob, "audio/webm;codecs=opus");

            IAsyncEnumerable<SpeechToTextResponseUpdate> streaming =
                _speech.GetStreamingTextAsync(audioData, speechToTextOptions);
    ...
    private async Task Dump(SpeechToTextResponseUpdate update)
    {
        var json = JsonSerializer.Serialize(update, _options) +
            Environment.NewLine;
        await File.AppendAllTextAsync("log_speech_async.json", json);
        if (update.Text != null)
        {
            Debug.WriteLine(json);
            Debug.WriteLine(string.Empty);
